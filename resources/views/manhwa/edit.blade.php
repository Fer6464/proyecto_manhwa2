    ...
    <div class="login_form">
        ...
        <form action="{{ route('manhwa.update', $obra->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="autor" class="form-control" value="{{ old('autor', $obra->autor) }}" required>
            </div>

            <div class="mb-3">
                <label for="portada" class="form-label">URL de la portada</label>
                <input type="url" name="portada" class="form-control" value="{{ old('portada', $obra->portada) }}">
            </div>

            <div class="mb-3">
                <label for="fecha_creacion" class="form-label">Fecha de creación</label>
                <input type="date" name="fecha_creacion" class="form-control"
                    value="{{ old('fecha_creacion', \Carbon\Carbon::parse($obra->fecha_creacion)->format('Y-m-d\TH:i')) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="date" name="fecha_finalizacion" class="form-control"
                    value="{{ old('fecha_finalizacion', $obra->fecha_finalizacion ? \Carbon\Carbon::parse($obra->fecha_finalizacion)->format('Y-m-d\TH:i') : '') }}"
                    id="fecha_finalizacion">
                <div class="form-check mt-2">
                    <input class="form-check-input" type="checkbox" id="sigueEmision" onchange="toggleFechaFinalizacion()" {{ !$obra->fecha_finalizacion ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea name="descripcion" class="form-control" required>{{ old('descripcion', $obra->descripcion) }}</textarea>
            </div>

            <div class="mb-3">
                <label for="estado" class="form-label">Estado</label>
                <select name="estado" class="form-control" required>
                    @foreach(['En emisión', 'Finalizado', 'En Hiatus', 'Cancelado'] as $estado)
                        <option value="{{ $estado }}" {{ old('estado', $obra->estado) === $estado ? 'selected' : '' }}>
                            {{ $estado }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="tags[]" class="form-control" multiple>
                    @foreach(App\Models\Tag::all() as $tag)
                        <option value="{{ $tag->id }}" {{ $obra->tags->contains($tag->id) ? 'selected' : '' }}>
                            {{ $tag->nombre }}
                        </option>
                    @endforeach
                </select>
